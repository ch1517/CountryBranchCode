...
function converterToCbc(codinate) {
  ...
  var code = [
    w[parseInt(parseInt(wP) / 100000)] + h[parseInt(parseInt(hP) / 100000)],
    parseInt((wP % 100000) / 10),
    parseInt((hP % 100000) / 10),
  ];
  return code;
}
// UTM-K를 WGS84로 변경
function converterToLatLng(cbcCode) {
  ...
  if (lat === null || lng === null || cbcCode[1].length !== cbcCode[2].length) {
    ...
    lat =
      Math.pow(10, length) * lat +
      parseInt(cbcCode[1]) * Math.pow(10, 6 - length) +
      5;
    lng =
      Math.pow(10, length) * lng +
...
function smallPointXY(m, minX, maxX, minY, maxY) {
  ...
  minX =
    parseInt(minX / m) * m < minX
      ? (parseInt(minX / m) - 1) * m
      : parseInt(minX / m) * m;
  minY =
    parseInt(minY / m) * m < minY
      ? (parseInt(minY / m) - 1) * m
      : parseInt(minY / m) * m;
  maxX =
    parseInt(maxX / m) * m < maxX
      ? (parseInt(maxX / m) + 1) * m
      : parseInt(maxX / m) * m;
  maxY =
    parseInt(maxY / m) * m < maxY
      ? (parseInt(maxY / m) + 1) * m
      : parseInt(maxY / m) * m;
