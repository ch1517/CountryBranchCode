...
const convertToCbc = (coordinate: [number, number]): [string, number, number] => {
  ...
  const wP: number = Number.parseInt(grs80P[0].toString().split('.')[0]);
  const hP: number = Number.parseInt(grs80P[1].toString().split('.')[0]);
...
const convertToLatLng = (_cbcCode: string): [number, number] | number => {
  ...
  Object.keys(w).forEach((key: string) => {
    if (w[Number.parseInt(key)] === cbcCode[0].charAt(0)) {
      lat = Number.parseInt(key);
    }
  });
  // "가나"의 "나" value의 key 확인
  Object.keys(h).forEach((key: string) => {
    if (h[Number.parseInt(key)] === cbcCode[0].charAt(1)) {
      lng = Number.parseInt(key);
    }
  });
  ...
  lat = 10 ** length * lat! + Number.parseInt(cbcCode[1]) * 10 ** (6 - length) + 5;
  lng = 10 ** length * lng! + Number.parseInt(cbcCode[2]) * 10 ** (6 - length) + 5;
...
const lineArray = (zoomLevel: number, latLngBounds: LatLngBounds) => {
  ...
  for (let index = 0; index < pArray.length; index++) {
    for (let index_ = 0; index_ < pArray[0].length; index_++) {
      if (index < pArray.length - 1 && index_ < pArray[index].length - 1) {
        const c = pArray[index][index_];
        // 정해진 bound 안에 있다면, 라벨과 grid 표시하기
        if (isWithinInnerBoundary(c)) {
          const nx = pArray[index + 1][index_];
          const ny = pArray[index][index_ + 1];
          const nxy = pArray[index + 1][index_ + 1];
          ...
          reArray.push({
            ...
            id: `${zoomLevel.toString()}.${pArray[index][index_]}y`,
            cbcText
          });
        }
      }
